...
const DiscoveryPage: React.FC = () => {
  ...
  const fetchDiscoveries = async () => {
    try {
      setLoading(true);
      const response = await fetch('https://api.taitongecopark.com/api/discovery');
      if (!response.ok) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const url = isEditing
        ? `https://api.taitongecopark.com/api/discovery/${formData.id}`
        : 'https://api.taitongecopark.com/api/discovery';
  ...
  const handleDelete = async (id: string) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`https://api.taitongecopark.com/api/discovery/${id}`, {
        method: 'DELETE'
      });
