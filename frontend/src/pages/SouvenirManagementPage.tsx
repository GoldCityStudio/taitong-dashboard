...
const SouvenirManagementPage: React.FC = () => {
  ...
  const fetchSouvenirs = async () => {
    setLoading(true);
    try {
      const response = await fetch('http://localhost:4000/api/souvenir');
      if (!response.ok) {
  ...
  const handleDelete = async (id: string) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`http://localhost:4000/api/souvenir/${id}`, {
        method: 'DELETE'
      });
  ...
  const handleSave = async () => {
    ...
    try {
      const url = isEditing
        ? `http://localhost:4000/api/souvenir/${currentSouvenir?.id}`
        : 'http://localhost:4000/api/souvenir';
